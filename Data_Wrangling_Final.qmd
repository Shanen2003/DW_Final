...
Here we swap the category of the Class column in the dataset in order to be able to use point biserial r (find the most and least important columns for acceptance or denial). 
```{python}
df.head()
...
From this we can see the most predictive columns are A8, A9, and then A10. Unfortunately the data set does nto say what these columns each stand for. Maybe I can reach out to credit card companies and ask for their input and piece it together. 
...
We now make the Class column into two categories, as it makes the future boxplotes look much better!
...
Here we can see the large gap in the A8 columns variables when comparing the 
```{python}
A8
```
...
We now make the Class column into two categories (again), as it makes the future boxplotes look much better!
